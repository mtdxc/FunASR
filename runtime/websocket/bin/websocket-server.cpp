  ...
  if(this->hotwords_embedding.empty()){
    ...
    if (jsonresult["hotwords"] != nullptr) {
      ...
      if (!json_string.empty()) {
        try {
          auto json_fst_hws = nlohmann::json::parse(json_string);
    ...
    if (!buffer.empty() && hotwords_embedding.size() > 0) {
      try{
        FUNASR_RESULT Result = FunOfflineInferBuffer(
            ...
            svs_lang, sys_itn);
        if (Result != nullptr) {
          auto text = FunASRGetResult(Result, 0);  // get decode result
          if (text) {
            jsonresult["text"] = text;            
          }
          if (text = FunASRGetStamp(Result)) {
            jsonresult["timestamp"] = text;
          }
          auto stamp_sents = FunASRGetStampSents(Result);
          if (stamp_sents) {
