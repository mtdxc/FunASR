...
struct FUNASR_MESSAGE {
  ...
  hv::EventLoopThreadPool* io_decoder = nullptr;
...
class WebSocketServer : public WebSocketService {
 public:
  WebSocketServer(hv::EventLoopThreadPool* pool) : io_decoder(pool){
      // set message handle
  ...
  void do_decoder(const std::vector<char>& buffer, WebSocketChannelPtr hdl,
